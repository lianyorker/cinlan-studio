...
export async function GET(request: Request) {
  if (!creativeCoreConfigured()) {
    return NextResponse.json({ generations: [], pagination: { page: 1, pageSize: 24, totalCount: 0, totalPages: 0, hasMore: false } })
  }
  try {
    ...
    if (url.searchParams.get('type') === 'video') {
      return NextResponse.json({ generations: [], pagination: { page, pageSize, totalCount: 0, totalPages: 0, hasMore: false } })
    }
