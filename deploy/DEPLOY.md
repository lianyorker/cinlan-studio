...
```bash
sha256sum /tmp/cinlan-studio-0.2.9-linux-source-20260911.tar.gz
```
...
```bash
sudo -u cinlan tar -xzf /tmp/cinlan-studio-0.2.9-linux-source-20260911.tar.gz \
...
`CREATIVE_ASSET_STORAGE_DIR` 必须允许 Web service 用户写入。`0.2.9` 会在该目录下自动创建 `.thumbnails`，其中只保存可重建的 WebP 预览缓存；原图仍保存在原有 `reference/`、`result/` 等目录中。
