  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4 backdrop-blur-sm" onClick={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <div className={`relative flex w-full max-w-3xl flex-col overflow-hidden rounded-lg bg-white shadow-2xl dark:bg-neutral-950 ${item.mediaType === 'text' ? 'h-[min(92dvh,860px)]' : 'max-h-[92dvh]'}`}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
        {item.mediaType === 'text' ? <div className="min-h-full p-7 pr-14"><MarkdownContent value={item.textResult || item.prompt || ''} className="text-sm leading-7" /></div> : item.mediaType === 'video' ? <video src={media} controls autoPlay muted className="max-h-[70vh] w-full object-contain" /> : <img src={media} alt="" className="max-h-[70vh] w-full object-contain" />}
      </div>
      <div className="shrink-0 space-y-3 border-t border-neutral-200 p-4 dark:border-neutral-800">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {media && item.mediaType === 'image' && <a href={media} download={`cinlan-${item.id}`} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-neutral-900 px-3 py-2.5 text-sm font-medium text-white transition hover:bg-neutral-800 dark:bg-white dark:text-neutral-900 dark:hover:bg-neutral-200"><IconDownload className="h-4 w-4" />{t.history.download}</a>}
